          ...
          .then((entries) => entries.forEach((key, val) => {
            ...
            return Promise.all(files.map((file) => {
              return nfs.fetch(file)
                ...
                .then((f) =>
                {
                  console.log("We are here okay with out file", file, f);
                  return f.read(CONSTANTS.FILE_READ.FROM_START, CONSTANTS.FILE_READ.TILL_END)
                    .catch( e => console.log("an error (not) thrown!", e))
                })
                .then((co) => {
                  console.log("success got the readddd!");
